...
class GenomicRDD(object):
    ...
    def rdd(self):
        """
        :return: Returns a PySpark RDD containing the underlying genomic data.
        :rtype: pyspark.rdd.RDD
        """

        return RDD(self._jvmRdd.jrdd(), self.sc)


    def _replaceRdd(self, newRdd):
        """
        Replaces the underlying RDD.

        Not implemented. Should be implemented by implementing classes.

        :param pyspark.rdd.RDD newRdd: The RDD to replace the current RDD with.
        :return: Returns a new RDD of the implementing class type.
        """

        return GenomicRDD(newRdd, self.sc)


    def transform(self, fn):
        """
        Transforms the underlying RDD by applying a function to it.

        :param fn: A function that maps an RDD into a new RDD.
        :return: Returns a new RDD of the implementing class type.
        """

        return self._replaceRdd(fn(self.rdd()))
