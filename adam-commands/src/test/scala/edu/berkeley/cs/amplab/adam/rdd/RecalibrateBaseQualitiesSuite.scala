...
import edu.berkeley.cs.amplab.adam.commands.{AdamCommand, AdamCommandCompanion, SparkArgs, SparkCommand}
import edu.berkeley.cs.amplab.adam.rdd.recalibration.DiscreteCycle
import scala.collection.mutable

object CompanionClass extends AdamCommandCompanion {

  val commandName = "used only for testing"
  val commandDescription = "used only for testing"

  def apply(c: Array[String]) : AdamCommand = null
}

class RecalibrateBaseQualitiesSuite extends FunSuite with BeforeAndAfterEach with SparkCommand {
  def run() : Unit = { }

  val companion = CompanionClass
  val context = createSparkContext(new SparkArgs {spark_master = "local"})
  ...
  test("Util :: RecalTable :: ErrorCount :: +=") {
    for ( numBaseCovar <- (1 to 50).map(_*100) ) {
      ...
      for ( i <- (1 to numBaseCovar) ) {
        ...
        if ( ! isMask ) { unFilteredSites += 1}
        if ( isMM && ! isMask ) { unFilteredMismatches += 1 }
  ...
  /*
  test("Covariate :: QualByRg :: Example") {
    ...
    val recRDD = context.makeRDD(records,1)
  ...
  test("Covariate :: QualByRG :: Combinatorial") {
    ...
    for ( numRG <- 1 to 50 ) {
      ...
      for ( numReads <- (1 to 50).map(t => t*10) ) {
        ...
        val recordsRDD = context.makeRDD(recsWithQuals.map(_._2).toList,1)
