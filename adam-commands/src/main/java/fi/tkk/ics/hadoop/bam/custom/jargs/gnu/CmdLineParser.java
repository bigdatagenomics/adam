// Based on git 7f672364.
//
// - Changed to use a List<String>-based instead of a String[]-based API.
// - Changed throws specifications to OptionException where it was marked that
...
public class CmdLineParser {
     ...
     */
    public final void parse( List<String> argv )
     ...
     * parsing options whose values might be locale-specific.
     */
    public final void parse( List<String> argv, Locale locale )
        throws OptionException {

        Vector<String> otherArgs = new Vector<String>();
        int position = 0;
        this.values = new Hashtable<String, Vector<Object>>(10);
        while ( position < argv.size() ) {
            String curArg = argv.get(position);
            if ( curArg.startsWith("-") ) {
                ...
                if ( opt.wantsValue() ) {
                    if ( valueArg == null ) {
                        position += 1;
                        if ( position < argv.size() ) {
                            valueArg = argv.get(position);
                        }
                    }
        ...
        for ( ; position < argv.size(); ++position ) {
            otherArgs.addElement(argv.get(position));
        }

        this.remainingArgs = new ArrayList<String>(otherArgs);
    }
    ...
    private Hashtable<String, Option> options =
        new Hashtable<String, Option>(10);
    private Hashtable<String, Vector<Object>> values;
}
