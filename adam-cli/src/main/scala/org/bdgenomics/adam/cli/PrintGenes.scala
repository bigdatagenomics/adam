...
class PrintGenesArgs extends Args4jBase with ParquetArgs with SparkArgs with Serializable {
...
class PrintGenes(protected val args: PrintGenesArgs)
  ...
  def run(sc: SparkContext, job: Job): Unit = {
    val features: RDD[GTFFeature] = sc.adamGTFFeatureLoad(args.gtfInput)
