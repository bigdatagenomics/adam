...
class CalculateDepth(protected val args: CalculateDepthArgs) extends ADAMSparkCommand[CalculateDepthArgs] {
  ...
  def run(sc: SparkContext, job: Job): Unit = {
    ...
    val adamRDD: RDD[Read] = sc.adamLoad(args.adamInputPath, projection = Some(proj))
    val mappedRDD = adamRDD.filter(_.getReadMapped)

    val seqDict = sc.adamDictionaryLoad[Read](args.adamInputPath)
    ...
    */
    ...
    val joinedRDD: RDD[(ReferenceRegion, Read)] =
