...
class MarkDuplicatesSuite extends SparkFunSuite {
  ...
  def createUnmappedRead() = {
    Read.newBuilder().setReadMapped(false).build()
  }
  ...
  private def markDuplicates(reads: Read*) = {
  ...
  test("quality scores") {
    ...
    val record = Read.newBuilder().setQual(qual).build()
