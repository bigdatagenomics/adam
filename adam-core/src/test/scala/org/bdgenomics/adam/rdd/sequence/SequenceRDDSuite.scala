...
class SequenceRDDSuite extends ADAMFunSuite {
  ...
  sparkTest("create a new sequence rdd") {
    val sequences: RDD[Sequence] = sc.parallelize(Seq(s1, s2))
    assert(SequenceRDD(sequences).rdd.count === 2)
  }
  ...
  sparkTest("save as parquet") {
    val sequences: SequenceRDD = SequenceRDD(sc.parallelize(Seq(s1, s2)))
    val outputPath = tempLocation(".adam")
    sequences.save(outputPath, asSingleFile = false)
  }
  ...
  sparkTest("save as single file fasta") {
    val sequences: SequenceRDD = SequenceRDD(sc.parallelize(Seq(s1, s2)))
    val outputPath = tempLocation(".fasta")
    sequences.save(outputPath, asSingleFile = true)
  }
