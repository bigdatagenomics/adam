...
import org.bdgenomics.formats.avro.{ FlatGenotype, Read }
...
object ReferenceMappingContext {
  ...
  implicit object ADAMFlatGenotypeReferenceMapping extends ReferenceMapping[FlatGenotype] with Serializable {
  ...
  implicit object ADAMRecordReferenceMapping extends ReferenceMapping[Read] with Serializable {
    def getReferenceName(value: Read): String =
      value.getContig.getContigName.toString

    def getReferenceRegion(value: Read): ReferenceRegion =
