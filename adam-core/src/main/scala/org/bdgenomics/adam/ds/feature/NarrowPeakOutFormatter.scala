 ...
 */
case class NarrowPeakOutFormatter() extends OutFormatter[Feature] {
  val narrowPeakParser = new NarrowPeakParser
  val stringency = ValidationStringency.STRICT

  /**
   * Reads features from an input stream in NarrowPeak format.
   *
  ...
  def read(is: InputStream): Iterator[Feature] = {
    ...
    @tailrec def convertIterator(iter: AsciiLineReaderIterator,
      ...
      if (!iter.hasNext) {
        ...
        val nextFeatures = narrowPeakParser.parse(iter.next, stringency).fold(features)(features += _)
