...
//
// Copied from parquet-mr release tag apache-parquet-1.10.1
// with modifications similar to https://github.com/apache/parquet-mr/pull/560
//
        ...
        switch(originalType) {
          case LIST:
            ...
            if (isElementType(repeatedType, parquetGroupType.getName())) {
              ...
              return Schema.createArray(convertField(repeatedType, names));
            } else {
              Type elementType = repeatedType.asGroupType().getType(0);
              if (elementType.isRepetition(Type.Repetition.OPTIONAL)) {
                return Schema.createArray(optional(convertField(elementType, names)));
              } else {
                return Schema.createArray(convertField(elementType, names));
              }
            }
          ...
          case MAP:
            ...
            if (valueType.isRepetition(Type.Repetition.OPTIONAL)) {
              return Schema.createMap(optional(convertField(valueType, names)));
            } else {
              return Schema.createMap(convertField(valueType, names));
            }
          case ENUM:
            return Schema.create(Schema.Type.STRING);
          case UTF8:
